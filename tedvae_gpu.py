...
class Model(PyroModule):
    ...
    def __init__(self, config):
        ...
        self.continuous_dim = config["continuous_dim"]
        self.contfeats = list(range(self.continuous_dim)) 
        self.binfeats = list(range(self.continuous_dim, self.continuous_dim + self.binary_dim))
...
class TEDVAE(nn.Module):
    def __init__(self, feature_dim, continuous_dim, binary_dim, outcome_dist="normal", 
        ...
        self.guide = Guide(config).to(device)
    ...
    def to_script_module(self):
        ...
        fake_x = torch.randn(2, self.feature_dim).to(self.device)
